...
public class PlayerMachine : SuperStateMachine {
    ...
    protected override void EarlyGlobalSuperUpdate()
    {
        ...
        lookDirection = Quaternion.AngleAxis(input.Current.MouseInput.x + input.Current.MouseInput.y, controller.up) * lookDirection;
    ...
    {
        Vector3 right = Vector3.Cross(controller.up, Vector3.up);
        Vector3 local = Vector3.zero;

        if (input.Current.MoveInput.x != 0)
        {
            local += Vector3.right * input.Current.MoveInput.x;
        }

        if (input.Current.MoveInput.z != 0)
        {
            local += Vector3.forward * input.Current.MoveInput.z;
        }
